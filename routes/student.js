...
router.get('/seat-management', async (req, res) => {
  try {
    ...
    // Get every seated student on this bus, keyed by their real seat_no
    const [seatedStudents] = await db.query(`
      SELECT
        s.*,
        u.name
      FROM students s
      JOIN users u
        ON s.user_id = u.id
      WHERE s.bus_id = ? AND s.seat_no IS NOT NULL
      ORDER BY s.id
    `, [student.bus_id]);

    const allocatedStudents = seatedStudents;
